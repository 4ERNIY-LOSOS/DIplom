      ...
      {loading ? (
        ...
        <>
                  ...
                  <TableBody>
                    {users.map((u) => (
                      <TableRow key={u.id} hover>
                        ...
                        <TableCell>
                          <Typography variant="body2">{u.company ? u.company.name : 'Без компании'}</Typography>
                          {u.company?.taxId && <Typography variant="caption" color="textSecondary">ИНН: {u.company.taxId}</Typography>}
                        </TableCell>
          ...
          {tab === 2 && (
            <Box>
              <Tabs 
                value={userFormData.taxId === 'internal_active' ? 0 : 1} 
                onChange={(_, v) => setUserFormData({...userFormData, taxId: v === 0 ? 'internal_active' : 'internal_history'})}
                sx={{ mb: 2 }}
              >
                <Tab label="Активные" />
                <Tab label="История" />
              ...
              <TableContainer component={Paper} variant="outlined">
                <Table>
                  <TableHead sx={{ bgcolor: '#f5f5f5' }}>
                    <TableRow><TableCell>Груз</TableCell><TableCell>Компания</TableCell><TableCell>Откуда - Куда</TableCell><TableCell>Статус</TableCell></TableRow>
                  </TableHead>
                  <TableBody>
                    {orders
                      .filter(o => (userFormData.taxId === 'internal_active' || !userFormData.taxId || userFormData.taxId === '') ? o.status !== 'Доставлено' : o.status === 'Доставлено')
                      .map((o) => (
                      <TableRow key={o.id}>
                        ...
                        <TableCell><Chip label={o.status} size="small" color={o.status === 'Доставлено' ? 'success' : 'primary'} /></TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
          ...
          {tab === 3 && (
            <Box>
              <Tabs 
                value={userFormData.taxId === 'ship_active' ? 0 : 1} 
                onChange={(_, v) => setUserFormData({...userFormData, taxId: v === 0 ? 'ship_active' : 'ship_history'})}
                sx={{ mb: 2 }}
              >
                <Tab label="Активные" />
                <Tab label="История" />
              ...
              <TableContainer component={Paper} variant="outlined">
                <Table>
                  <TableHead sx={{ bgcolor: '#f5f5f5' }}>
                    <TableRow><TableCell>Заявка</TableCell><TableCell>Транспорт</TableCell><TableCell>Водитель</TableCell><TableCell>Статус</TableCell></TableRow>
                  </TableHead>
                  <TableBody>
                    {shipments
                      .filter(s => (userFormData.taxId === 'ship_active' || !userFormData.taxId || userFormData.taxId === '') ? s.status !== 'Доставлено' : s.status === 'Доставлено')
                      .map((s) => (
                      <TableRow key={s.id}>
                  ...
                  <TableBody>
                    {vehicles.map((v) => (
                      <TableRow key={v.id}>
                        ...
                        <TableCell><Chip label={v.status} size="small" color={v.status === 'Свободен' ? 'success' : 'warning'} /></TableCell>
                        <TableCell align="right">
          ...
          {tab === 5 && (
            <Box>
              ...
              <TableContainer component={Paper} variant="outlined">
                <Table>
                  <TableHead sx={{ bgcolor: '#f5f5f5' }}>
                    <TableRow><TableCell>ФИО</TableCell><TableCell>Лицензия</TableCell><TableCell>Категория</TableCell><TableCell>Телефон</TableCell><TableCell>Статус</TableCell><TableCell align="right">Действия</TableCell></TableRow>
                  </TableHead>
                  <TableBody>
                    {drivers.map((d) => (
                      <TableRow key={d.id}>
                        <TableCell>{d.fullName}</TableCell><TableCell>{d.licenseNumber}</TableCell><TableCell>{d.licenseCategory || '—'}</TableCell><TableCell>{d.phone}</TableCell>
                        <TableCell><Chip label={d.status} size="small" color={d.status === 'Свободен' ? 'success' : 'warning'} /></TableCell>
                        <TableCell align="right">
