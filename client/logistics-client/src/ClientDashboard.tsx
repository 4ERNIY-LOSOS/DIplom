...
import { 
  ...
  Stepper, Step, StepLabel, Divider, Snackbar, Alert, Tabs, Tab, Chip
} from '@mui/material';
...
export default function ClientDashboard() {
  ...
  const historyOrders = orders.filter(o => o.status === 'Доставлено');
    ...
    <LocalizationProvider dateAdapter={AdapterDayjs}>
      <Paper sx={{ p: 4, minHeight: '80vh', borderRadius: 2 }}>
        ...
        <Tabs value={tabValue} onChange={(_, v) => setTabValue(v)} sx={{ mb: 3 }}>
          <Tab label={`Активные (${activeOrders.length})`} />
          <Tab label={`История (${historyOrders.length})`} />
        </Tabs>
        ...
        <TableContainer>
          <Table>
            ...
            <TableBody>
              {(tabValue === 0 ? activeOrders : historyOrders).map((o: any) => (
                <TableRow key={o.id} hover>
                  <TableCell>{o.cargoName}</TableCell>
                  <TableCell>
                    <Chip 
                      label={getStatusLabel(o.status)} 
                      color={o.status === 'Доставлено' ? 'success' : 'primary'} 
                      size="small" 
                    />
                  </TableCell>
              ...
              {(tabValue === 0 ? activeOrders : historyOrders).length === 0 && (
                <TableRow>
                  <TableCell colSpan={3} align="center">
                    <Typography variant="body2" sx={{ py: 3, color: 'text.secondary' }}>
                      {tabValue === 0 ? 'Нет активных заявок' : 'История пуста'}
                    </Typography>
                  </TableCell>
                </TableRow>
              )}
          ...
          <DialogContent>
            {selectedOrder && (
              <Box sx={{ mt: 2 }}>
                ...
                {(selectedOrder.status === 'Запланировано' || selectedOrder.status === 'Подтверждено' || selectedOrder.status === 'В пути' || selectedOrder.status === 'Доставлено') && (
                  <Box sx={{ mt: 2, bgcolor: '#f0f7ff', p: 2, borderRadius: 1 }}>
                    <Typography variant="subtitle2" color="primary" sx={{ mb: 1 }}>ИНФОРМАЦИЯ О ДОСТАВКЕ:</Typography>
                    
                    {selectedOrder.shipment && (
                      <Box sx={{ mb: 1, pb: 1, borderBottom: '1px dashed #ccc' }}>
                        <Typography><strong>Транспорт:</strong> {selectedOrder.shipment.vehicle?.model} ({selectedOrder.shipment.vehicle?.plateNumber})</Typography>
                        <Typography><strong>Водитель:</strong> {selectedOrder.shipment.driver?.fullName}</Typography>
                      </Box>
                    )}
                    ...
                    {(selectedOrder.status === 'Доставлено' || selectedOrder.deliveryStartDate) && (
                      <Typography><strong>Начало доставки:</strong> {selectedOrder.deliveryStartDate ? new Date(selectedOrder.deliveryStartDate).toLocaleDateString() : '---'}</Typography>
                    )}
                    
                    {(selectedOrder.status === 'Доставлено' || selectedOrder.deliveryEndDate) && (
                      <Typography><strong>Конец доставки:</strong> {selectedOrder.deliveryEndDate ? new Date(selectedOrder.deliveryEndDate).toLocaleDateString() : '---'}</Typography>
                    )}
                  </Box>
                )}
