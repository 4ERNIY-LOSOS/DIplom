...
export class ShipmentsService {
  ...
  async updateStatus(id: number, status: string, userId?: number) {
    ...
    if (status === 'Доставлено') {
        ...
        if (fullShipment) {
            await this.vehiclesRepository.update(fullShipment.vehicle.id, { status: 'Свободен' });
            await this.driversRepository.update(fullShipment.driver.id, { status: 'Свободен' });
        }
    }
    ...
    if (shipment.orders) {
      ...
      const now = new Date();
      if (status === 'Запланировано' || status === 'Подтверждено') {
        updateData.pickupDate = now;
      } else if (status === 'В пути') {
        updateData.deliveryStartDate = now;
      } else if (status === 'Доставлено') {
        updateData.deliveryEndDate = now;
      }

      await this.ordersRepository.update(orderIds, updateData);
    }
